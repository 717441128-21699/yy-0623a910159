...
import type { ConsentRecord, ConsentStatus, Patient, TreatmentItem } from '@/types';
import { generateId } from '@/utils/id';
import { nowISO } from '@/utils/date';

const STORAGE_KEY = 'dental-consent-records-v1';
...
interface ConsentState {
  ...
  markSigned: (id: string, signatureData: string) => void;
...
export const useConsentStore = create<ConsentState>((set, get) => ({
  ...
  markSigned: (id, signatureData) => {
    const records = get().records.map((r) =>
      r.id === id
        ? { ...r, status: 'signed' as ConsentStatus, signatureData, signedAt: nowISO() }
        : r,
    );
    saveToStorage(records);
    set({ records });
  ...
  seedDemoData: () => {
    ...
    const items = [
      { id: 'implant', name: '种植牙', code: 'IMPLANT', icon: 'Flower2', description: '', defaultFee: '12,800 元/颗' },
      { id: 'extraction', name: '拔牙', code: 'EXTRACTION', icon: 'Scissors', description: '', defaultFee: '800 元' },
      { id: 'root-canal', name: '根管治疗', code: 'ROOT_CANAL', icon: 'CircleDot', description: '', defaultFee: '1,500 元' },
      { id: 'orthodontics', name: '正畸矫正', code: 'ORTHODONTICS', icon: 'Grid3x3', description: '', defaultFee: '28,000 元' },
      { id: 'filling', name: '补牙', code: 'FILLING', icon: 'ShieldCheck', description: '', defaultFee: '500 元' },
      { id: 'cleaning', name: '洗牙洁治', code: 'CLEANING', icon: 'Sparkles', description: '', defaultFee: '300 元' },
    ] as TreatmentItem[];
    ...
    const demo: ConsentRecord[] = demoPatients.map((p, i) => {
      ...
      return {
        ...
        status: s,
        signatureData: s === 'signed' ? null : null,
